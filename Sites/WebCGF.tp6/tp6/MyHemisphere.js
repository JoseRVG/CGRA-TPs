	...
	for(var k = 0; k <= this.stacks; k++)
	{
		...
		var alpha = 0;
			...
			for(var i = 0; i <= this.slices; i++)
			{
				...
				this.texCoords.push(i/(this.slices),-k/(this.stacks) + 1);
	...
	for(var k = 0; k < this.stacks; k++)
	{
		...
		var alpha = 0;
			...
			for(var i = 0; i < this.slices; i++)
			{
				...
				//Generate the texture coords
				//this.texCoords.push(0.5*Math.cos(phiRad)*Math.cos(alphaRad) + 0.5,-0.5*Math.cos(phiRad)*Math.sin(alphaRad) + 0.5);
				this.texCoords.push(i/(this.slices-1),k/(this.stacks-1));
