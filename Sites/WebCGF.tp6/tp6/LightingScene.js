...
LightingScene.prototype.initCameras = function() {
	this.camera1Pos = vec3.fromValues(15, 15, 15);
	this.camera1Dir = vec3.fromValues(0,0,0);

	this.camera2Pos = vec3.fromValues(7,5,1);
	this.camera2Dir = vec3.fromValues(7,5,2);

	this.camera3Pos = vec3.fromValues(15,1,15);
	this.camera3Dir = vec3.fromValues(0,1,0);

	this.camera = new CGFcamera(0.9, 0.1, 500, this.camera1Pos, this.camera1Dir);
	this.camUsed = 1;
};
...
LightingScene.prototype.update = function(currTime) {
	...
	if(this.usedCam == 1)
	{
		vec3.copy(this.camera1Pos,this.camera.position);
		vec3.copy(this.camera1Dir, this.camera.target);
	}
	else if(this.usedCam == 2)
	{
		vec3.copy(this.camera2Pos,this.camera.position);
		vec3.copy(this.camera2Dir, this.camera.target);
	}
	else if(this.usedCam == 3)
	{
		vec3.copy(this.camera3Pos,this.camera.position);
		vec3.copy(this.camera3Dir, this.camera.target);
	}
...
LightingScene.prototype.processQDown = function() {
	if(this.camUsed == 1)
	{
		this.camUsed = 0;
		this.drone2.setUsed(true);
	}
	else
	{
		this.camUsed = 1;
		this.drone2.setUsed(false);
		vec3.copy(this.camera.position,this.camera1Pos);
		vec3.copy(this.camera.target,this.camera1Dir);
	}
}

LightingScene.prototype.processEDown = function() {
	if(this.camUsed == 2)
	{
		this.camUsed = 0;
		this.drone2.setUsed(true);
	...
	else
	{
		this.camUsed = 2;
		this.drone2.setUsed(false);
		vec3.copy(this.camera.position,this.camera2Pos);
		vec3.copy(this.camera.target,this.camera2Dir);
	}
}

LightingScene.prototype.processRDown = function() {
	if(this.camUsed == 3)
	{
		this.camUsed = 0;
		this.drone2.setUsed(true);
	...
	else
	{
		this.camUsed = 3;
		this.drone2.setUsed(false);
		vec3.copy(this.camera.position,this.camera3Pos);
		vec3.copy(this.camera.target,this.camera3Dir);
	}
}